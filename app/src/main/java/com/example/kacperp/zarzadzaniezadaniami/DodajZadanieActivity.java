...
public class DodajZadanieActivity extends AppCompatActivity {
    ...
    public void DodajZadanie(View view)
    {
        try {
        SharedPreferences mPrefs = getSharedPreferences("IDvalue", 0);
        String IdUsera = mPrefs.getString("idusera", "");
        ...
        if(Termin.getText().toString().length() == 10 ) {
            ...
            String connString = "http://192.168.0.11:3466/api/zadania";
            final AsyncHttpClient client = new AsyncHttpClient();
            ...
            client.post(connString, rp, new AsyncHttpResponseHandler() {
                ...
                public void onSuccess(int statusCode, Header[] headers, byte[] response) {
                    ...
                    if (responseString.contains("OK")) {
                        Context context = getApplicationContext();
                        Toast toast = Toast.makeText(context, "DODANO ZADANIE", Toast.LENGTH_SHORT);
                        toast.show();
                    } else {
                        Context context = getApplicationContext();
                        Toast toast = Toast.makeText(context, "Błąd przy dodawaniu", Toast.LENGTH_SHORT);
                        toast.show();
                    }
